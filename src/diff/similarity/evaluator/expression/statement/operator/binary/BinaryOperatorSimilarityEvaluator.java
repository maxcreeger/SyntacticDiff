...
public class BinaryOperatorSimilarityEvaluator extends SimilarityEvaluator<BinaryOperator> {
	...
	public Similarity eval(BinaryOperator operator1, BinaryOperator operator2) {
		...
		Similarity operatorSim = new LeafSimilarity<BinaryOperator>("BinaryOp", sameOperator ? 1 : 0, 1, operator1, operator2) {

			@Override
			public List<String[]> show(String prefix) {
				List<String[]> list = new ArrayList<>();
				list.add(new String[] { prefix + operator1.getOperatorSymbol(), Double.toString(similarity()), prefix + operator2.getOperatorSymbol() });
				return list;
			}
		...
		// TODO try to switch left-right / right-left
