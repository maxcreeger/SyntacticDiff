...
public final class CatchBlock implements Showable {
	...
	private final ShowableString catchKeyword;
	...
	private static Optional<CatchBlock> buildOne(CodeBranch input) {
		...
		// Match 'catch' keyword
		Optional<ShowableString> catchKeyword = ShowableString.fromPattern(fork, CATCH_PATTERN);
		if (!catchKeyword.isPresent()) {
		...
		return Optional.of(new CatchBlock(catchKeyword.get(), exceptionTypes, exceptionReference.get(), catchExpressions, fork.commit()));
	}
	...
	public List<String> fullBreakdown(String prefix) {
		...
		StringBuilder exceptionTypesList = new StringBuilder(
			prefix + SyntacticSettings.red() + SyntacticSettings.bold() + "catch" + SyntacticSettings.reset() + " (");
		...
		for (Expression expression : catchExpressions) {
			res.addAll(expression.fullBreakdown(prefix + "h  "));
		}
		res.add(prefix + "} ");
