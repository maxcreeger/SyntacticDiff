...
/**
 * Detects java imports like "import java.util.List;" or "import
 * lexer.usual.structure.SingleTokenStructure.*"
 */
public class ImportStatementDetector implements StructureLexer<JavaGrammar, ImportStatement> {

	// Basic tokens
	private static final SingleWordStructureLexer<JavaGrammar> PACKAGE_NAME_LEXER = new SingleWordStructureLexer<JavaGrammar>(
		token -> token.get().matches("[a-z0-9]+"));
	private static final SingleWordStructureLexer<JavaGrammar> CLASS_NAME_LEXER = new SingleWordStructureLexer<JavaGrammar>(
		token -> token.get().matches("[A-Z][A-Za-z0-9]*"));
	...
	private static final StructureLexer<JavaGrammar, RepeatStructure<JavaGrammar, SequenceStructure<JavaGrammar>>> REPEAT_DOT_PACKAGE_LEXER = new RepeatStructureFinder<JavaGrammar, SequenceStructure<JavaGrammar>, SequenceStructureLexer<JavaGrammar>>(
		DOT_PACKAGE);
	// Any number of sequences ".Class1.Class2" including zero sequences
	private static final StructureLexer<JavaGrammar, RepeatStructure<JavaGrammar, SequenceStructure<JavaGrammar>>> REPEAT_DOT_CLASS_LEXER = new RepeatStructureFinder<JavaGrammar, SequenceStructure<JavaGrammar>, SequenceStructureLexer<JavaGrammar>>(
		DOT_CLASS);
	// An optional '.*' sequence
	private static final OptionalStructureFinder<JavaGrammar, SequenceStructure<JavaGrammar>> OPTIONAL_DOT_STAR_LEXER = new OptionalStructureFinder<JavaGrammar, SequenceStructure<JavaGrammar>>(
		DOT_STAR);
	...
	public static void main(String[] argc) {
		...
		TokenStream<JavaGrammar> longTokenStream = TokenStream.of(JavaCodeTokenizer.TOKENIZER.tokenizeAll(new AtomicReference<String>(longImport)).get());
