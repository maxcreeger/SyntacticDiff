...
public class SequenceStructure<G extends Grammar> implements Structure<G> {
	...
	/**
	 * Builds a sequence {@link Structure}. Use a {@link SequenceStructureLexer}
	 * preferably.
	 */
	 ...
	 *            the expected type
	 * @param expectedClass
	 *            the expected {@link Class} reference
	 * @param index
	...
	public static class SequenceStructureLexer<G extends Grammar> implements StructureLexer<G, SequenceStructure<G>> {
		 ...
		 * 
		 * @param <S>
		 *            an expected {@link Structure} type
		 * @param structureClass
		 *            the class of expected structure
		 * @param lexer
		 *            the lexer
		 */
	...
	public static void main(String[] argc) {
		...
		StructureLexer<JavaGrammar, Symbol<JavaGrammar>> importLexer = new SingleSymbolStructureLexer<JavaGrammar>(token -> "import".equals(token.get()));
		...
		final List<Token<JavaGrammar>> tokenList = JavaCodeTokenizer.TOKENIZER.tokenizeAll(new AtomicReference<String>("import java;")).get();
