...
/**
 * Represents a word (that is not a known, system-reserved {@link Symbol} of the {@link Grammar}.
 *
...
public interface Word<G extends Grammar> extends Token<G>, SingleTokenStructure<G> {
    ...
    public static class WordImpl<G extends Grammar> implements Word<G> {
        ...
        public WordImpl(CodeLocation location) {
            this.location = location;
            this.word = location.getCode();
        }
    ...
    static class WordBuilder<G extends Grammar> implements TokenBuilder<G, Word<G>> {
        @Override
        public Word<G> build(CodeLocation code) {
            return new WordImpl<G>(code);
        }
    }
